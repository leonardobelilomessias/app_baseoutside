...
import { FormatServiceAgent } from "./IFormatsService/IFormatServiceAgent"
...
class HandleDataAgent {
    ...
    async fetchAgentById(id_agent: string): Promise<FullAgentDTO> {
        const response = await this.service.fetchDataAgentById(id_agent)
        return response
